...
namespace Controllers.Inputs
{
    ...
    public static class MouseController
    {
        ...
        public static bool IsScrollingUp() => _scrollThisFrame > 0;
        public static bool IsScrollingDown() => _scrollThisFrame < 0;
        ...
        public static bool GetMouseOverlap(GameObject obj)
        {
            ...
            if (obj._hitbox == null) return false;

            Hitbox hitbox = obj._hitbox;

            if (!hitbox._collisionFlags.TryGetValue(CollisionFlag.MOUSE, out bool canCollide) || !canCollide) return false;

            return hitbox._type switch
            {
                HitboxType.RECTANGLE =>
                    mousePos.X >= obj._position.X && mousePos.X <= obj._position.X + obj._size.X &&
                    mousePos.Y >= obj._position.Y && mousePos.Y <= obj._position.Y + obj._size.Y,

                HitboxType.CIRCLE =>
                    Vector2.Distance(mousePos, obj._position + obj._size / 2) <= obj._size.X, // assuming size.X == radius
