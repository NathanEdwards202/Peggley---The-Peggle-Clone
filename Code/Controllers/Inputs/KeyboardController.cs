...
namespace Controllers.Inputs
{
    public static class KeyboardController
    {
        ...
        static HashSet<Keys> _lastFramePressedKeys = new();
        ...
        public static void AltUpdate(double deltaTime)
        {
            HashSet<Keys> currentPressedKeys = new(Keyboard.GetState().GetPressedKeys());

            // Identify keys that were released this frame
            foreach (Keys key in _lastFramePressedKeys.Except(currentPressedKeys))
            {
                // This key was released, apply UnpressedLogic
                UnpressedLogic((int)key);
            }

            // Identify keys that are now pressed and update their durations
            foreach (Keys key in currentPressedKeys)
            {
                _keyPressDuration[(int)key] += deltaTime;
            }

            // Update the set of keys pressed for the next frame
            _lastFramePressedKeys = currentPressedKeys;
        }
